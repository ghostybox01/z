...
def _email_provider(domain: str) -> str:
    ...
    if "gmail" in d:
        return "Gmail"
    if any(x in d for x in ["outlook", "hotmail", "live.com", "msn"]):
        return "Outlook"
    if "yahoo" in d:
        return "Yahoo"
    if "icloud" in d or "me.com" in d:
        return "Apple"
    if "aol" in d:
        return "AOL"
    if "proton" in d:
        return "Proton"
    if d in _FREE_PROVIDERS:
...
def _make_registry(ctx: dict) -> list[tuple[str, Any]]:
    ...
    entries = [
        ...
        *[(f"#FIELD_{k.upper()}", v) for k, v in ctx["custom"].items()],
        ...
        (
            "#MD5_EMAIL",
            lambda _: hashlib.md5(email.encode(), usedforsecurity=False).hexdigest(),
        ),
        (
            "#SHA1_EMAIL",
            lambda _: hashlib.sha1(email.encode(), usedforsecurity=False).hexdigest(),
        ),
        ...
        (
            "#HEXEMAIL",
            lambda _: __import__("binascii").hexlify(email.encode()).decode(),
        ),
        # ── RANDOM DATA ───────────────────────────────────────────────
        ...
        (
            "#RAND_LOCATION_CITY_COUNTRY",
            lambda _: "{}, {}".format(*random.choice(CITY_COUNTRY_PAIRS)),
        ),
        (
            "#RAND_LOCATION_COUNTRY_CITY",
            lambda _: "{}, {}".format(
                random.choice(CITY_COUNTRY_PAIRS)[1],
                random.choice(CITY_COUNTRY_PAIRS)[0],
            ),
        ),
        ...
        (
            "#RANDFULLNAME",
            lambda _: (
                f"{random.choice(FAKE_NAMES_FIRST)} {random.choice(FAKE_NAMES_LAST)}"
            ),
        ),
        ...
        (
            "#JWT_FAKE",
            lambda _: (
                f"eyJ{_rand_alphanum(20)}.eyJ{_rand_alphanum(40)}.{_rand_alphanum(43)}"
            ),
        ),
        ...
        (
            "#VERSION_NUM",
            lambda _: (
                f"{random.randint(1, 9)}.{random.randint(0, 15)}.{random.randint(0, 30)}"
            ),
        ),
        (
            "#SEMVER",
        ...
        (
            ...
            lambda _: (
                lambda fn, co: (
                    f"{fn.lower()}@{co.lower().replace(' ', '').replace('.', '').replace(',', '')[:12]}.com"
                )
        ...
        (
            ...
            lambda _: (
                lambda fn, ln: (
                    f"{fn.lower()}{random.choice(['.', '_', ''])}{ln.lower()}@{_rand_domain()}"
                )
        ...
        (
            "#RANDOM_LINK",
            lambda _: (
                f"https://{_rand_domain()}/ref/{_rand_alphanum(4)}/{_rand_alphanum(8)}"
            ),
        ),
            ...
            str(
                (
                    _date(
                        now.year,
                        now.month,
                        _calendar.monthrange(now.year, now.month)[1],
                    )
                    - now.date()
                ).days
            ),
        ...
        (
            "#IS_OUTLOOK",
            "true"
            if any(x in domain.lower() for x in ["outlook", "hotmail", "live.com"])
            else "false",
        ),
...
def _apply_regex_tags(s: str, ctx: dict) -> str:
    ...
    s = re.sub(
        r"#RAND_LOCATION_CITY_COUNTRY", lambda m: _rand_location("city_country"), s
    )
    s = re.sub(
        r"#RAND_LOCATION_COUNTRY_CITY", lambda m: _rand_location("country_city"), s
    )
    ...
    s = re.sub(r"#RANDNUM\{(\d+)\}", lambda m: _rand_digits(_safe_int(m.group(1))), s)
    s = re.sub(r"#RANDNUM(\d+)", lambda m: _rand_digits(_safe_int(m.group(1))), s)
    s = re.sub(
        r"#RANDALPHANUM\{(\d+)\}", lambda m: _rand_alphanum(_safe_int(m.group(1))), s
    )
    s = re.sub(
        r"#RANDALPHANUM(\d+)", lambda m: _rand_alphanum(_safe_int(m.group(1))), s
    )
    s = re.sub(r"#RANDALPHA\{(\d+)\}", lambda m: _rand_alpha(_safe_int(m.group(1))), s)
    s = re.sub(r"#RANDALPHA(\d+)", lambda m: _rand_alpha(_safe_int(m.group(1))), s)
    s = re.sub(r"#RANDHEX\{(\d+)\}", lambda m: _rand_hex(_safe_int(m.group(1))), s)
    s = re.sub(r"#RANDHEX(\d+)", lambda m: _rand_hex(_safe_int(m.group(1))), s)
    s = re.sub(
        r"#RANDUPPERALPHA\{(\d+)\}",
        lambda m: _rand_alpha(_safe_int(m.group(1))).upper(),
        s,
    )
    s = re.sub(
        r"#RANDUPPERALPHA(\d+)", lambda m: _rand_alpha(_safe_int(m.group(1))).upper(), s
    )
    s = re.sub(
        r"#RANDUPPERHEX\{(\d+)\}", lambda m: _rand_hex(_safe_int(m.group(1))).upper(), s
    )
    s = re.sub(
        r"#RANDUPPERHEX(\d+)", lambda m: _rand_hex(_safe_int(m.group(1))).upper(), s
    )
    s = re.sub(
        r"#RANDUPPERAL\{(\d+)\}",
        lambda m: _rand_alphanum(_safe_int(m.group(1))).upper(),
        s,
    )
    s = re.sub(
        r"#RANDUPPERAL(\d+)", lambda m: _rand_alphanum(_safe_int(m.group(1))).upper(), s
    )
    s = re.sub(
        r"#RANDOMSTR\{(\d+)\}", lambda m: _rand_alphanum(_safe_int(m.group(1))), s
    )
    s = re.sub(r"#RANDOMSTR(\d+)", lambda m: _rand_alphanum(_safe_int(m.group(1))), s)
    ...
    if links_cfg and links_cfg.get("links"):
        valid = [link["url"] for link in links_cfg["links"] if link.get("url")]
        if valid:
...
def resolve_tags(text: str, ctx: dict) -> str:
    ...
    def _date_off(m):
        try:
            return _fmt_date(_now + timedelta(days=int(m.group(1))))
        except Exception:
            return ""

    def _dateshort_off(m):
        try:
            return _fmt_date_short(_now + timedelta(days=int(m.group(1))))
        except Exception:
            return ""

    def _dateiso_off(m):
        try:
            return _fmt_date_iso(_now + timedelta(days=int(m.group(1))))
        except Exception:
            return ""
...
def validate_tags(
    *texts: str,
    ...
    if ctx and lead_tags:
        ...
        if not email:
            errors.append(
                "Lead is missing an email address — all #EMAIL/* tags will be empty"
            )
        if not name and any(
            t in lead_tags for t in ["#REALNAME", "#FIRSTNAME", "#LASTNAME", "#IF_NAME"]
        ):
            warnings.append(
                "Lead has no name — #REALNAME, #FIRSTNAME will fall back to email username"
            )
        if not company and "#COMPANY" in lead_tags:
            info.append(
                "#COMPANY will use domain name as fallback (no company field in lead)"
            )
    ...
    for text in texts:
        ...
        for tag in open_braces:
            errors.append(
                f"Unclosed brace in tag: {tag[:50]}... — closing '}}' is missing"
            )
        ...
        for opts in empty_shuf:
            ...
            if len(parts) == 0:
                errors.append(
                    "#SHUF{} has no options — provide at least one: #SHUF{option1|option2}"
                )
            elif len(parts) == 1:
                warnings.append(
                    f"#SHUF{{{opts}}} has only one option — shuffle has nothing to randomize"
                )
        ...
        for n in big_repeats:
            if int(n) > 50:
                warnings.append(
                    f"#REPEAT{{...}}{{{n}}} — repeat count {n} is very high, may bloat email size"
                )
        ...
        for n in big_gens:
            if int(n) > 100:
                warnings.append(
                    f"#RAND...{n} — generating {n} characters; values >100 chars may look suspicious"
                )
    ...
    for tag in unknown_tags:
        ...
        warnings.append(
            f"Unknown tag: {tag} — will be left as literal text in the email"
        )
    ...
    if ctx:
        ...
        if not ctx.get("from_name") and "#FROMNAME" in used_tags:
            warnings.append(
                "#FROMNAME tag used but sender has no From Name — will be empty"
            )
    ...
    return {
        ...
        "clashes": [f"{src} / {dst}" for src, dst in clashes],
    }
...
TAG_CATALOG = [
    {
        ...
        "tags": [
            ...
            {
                "tag": "#COMPANY",
                "desc": "Company from lead list, or capitalized domain",
            },
            {"tag": "#COMPANY_UPPER", "desc": "Company in UPPERCASE"},
            {
                "tag": "#DOMAIN_LOGO_URL",
                "desc": "Clearbit logo image URL for recipient domain",
            },
            {
                "tag": "#FIELD_{KEY}",
                "desc": "Any custom lead column, e.g. #FIELD_PHONE",
            },
        ],
    },
    {
    ...
    },
    {
        ...
        "tags": [
            ...
            {
                "tag": "#DATE_IN{N}",
                "desc": "Full date N days from now — e.g. #DATE_IN{7}, #DATE_IN{-3}",
            },
            {
                "tag": "#DATE_PLUS{N}",
                "desc": "Full date N days from now (alias for DATE_IN)",
            },
            {
                "tag": "#DATESHORT_IN{N}",
                "desc": "Short date N days from now — MM/DD/YYYY",
            },
    ...
    },
    {
        ...
        "tags": [
            ...
            {
                "tag": "#RANDPHONE_INTL",
                "desc": "Random international phone +44 123 456 7890",
            },
    ...
    },
    {
        ...
        "tags": [
            ...
            {
                "tag": "#FAKE_COMPANY_EMAIL",
                "desc": "Fake company email — name@company.com",
            },
            {
                "tag": "#FAKE_FULLNAME_EMAIL",
                "desc": "Fake person email — john.smith@domain.com",
            },
    ...
    },
    {
        ...
        "tags": [
            {
                "tag": "#SHUF{A|B|C}",
                "desc": "Pick random option from pipe-separated list",
            },
            {"tag": "#IF_NAME{text}", "desc": "Show text only if lead has a name"},
            {"tag": "#IF_NONAME{text}", "desc": "Show text only if lead has NO name"},
            {
                "tag": "#IF_COMPANY{text}",
                "desc": "Show text only if lead has a company",
            },
            {
                "tag": "#IF_NOCOMPANY{text}",
                "desc": "Show text only if lead has NO company",
            },
            {
                "tag": "#IF_FIELD{key}{text}",
                "desc": "Show text if lead has custom field 'key'",
            },
            ...
            {
                "tag": "#PADLEFT{text}{N}{c}",
                "desc": "Left-pad text to N chars with char c",
            },
            {
                "tag": "#PADRIGHT{text}{N}{c}",
                "desc": "Right-pad text to N chars with char c",
            },
