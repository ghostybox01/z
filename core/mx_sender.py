    ...
    def record_fail(self, from_email: str, error_str: str):
        ...
        with self._lock:
            ...
            if count >= self._max_fails:
                ...
                log.warning(
                    "[SenderHealth] %s marked dead after %d consecutive from-address failures",
                    from_email,
                    count,
                )
    ...
    if not mx_hosts:
        ...
        try:
            out = _sp.check_output(
                ["nslookup", "-type=MX", domain], timeout=8, stderr=_sp.DEVNULL
...
def _connect_mx(
    ...
    if socks_proxy and socks_proxy.get("host"):
        try:
            ...
            if code != 220:
                msg_str = (
                    msg_b.decode(errors="replace")
                    if isinstance(msg_b, bytes)
                    else str(msg_b)
                )
                raise smtplib.SMTPConnectError(
                    code, f"SMTP banner: {code} {msg_str[:80]}"
                )
        except ImportError:
            raise Exception(
        ...
        smtp = smtplib.SMTP(mx_host, 25, timeout=connect_timeout, local_hostname=ehlo)
        if smtp.sock:
...
def send_direct_mx(
    ...
    ehlo = ehlo_domain or (
        from_email.split("@")[-1] if "@" in from_email else "mail.server.local"
    )
    ...
    if health.is_dead(from_email):
        raise Exception(
            f"Sender {from_email} is dead — too many from-address rejections"
        )
    ...
    if domain in STRICT_DOMAINS:
        min_delay = (
            MS_DOMAIN_MIN_DELAY if domain in MS_DOMAINS else STRICT_DOMAIN_MIN_DELAY
        )
        ...
        try:
            ...
            if any(s in err_low for s in _RCPT_PERMANENT_SIGNALS):
                ...
                errors_detail.append(
                    f"{mx_host}(content_block): {code} {err_str[:120]}"
                )
                tracker.record_fail(domain, err_str)
                break
                ...
                errors_detail.append(
                    f"{mx_host}(sender_policy): {code} {err_str[:120]}"
                )
                ...
                if extra_senders:
                    for alt_sender in extra_senders:
                        ...
                        try:
                            ...
                            smtp2.send_message(
                                msg, from_addr=alt_email, to_addrs=[lead_email]
                            )
                            _close_smtp(smtp2)
                            tracker.record_send(domain)
                            health.record_success(alt_email)
                            log.info(
                                "[MxSender] fallback sender %s succeeded for %s",
                                alt_email,
                                domain,
                            )
                            return mx_host
                ...
                for retry in range(TRANSIENT_MAX_RETRIES):
                    ...
                    log.info(
                        "[MxSender] %s transient on %s — retry %d in %ds",
                        domain,
                        mx_host,
                        retry + 1,
                        delay,
                    ...
                    try:
                        ...
                        smtp2.send_message(
                            msg, from_addr=from_email, to_addrs=[lead_email]
                        )
                        _close_smtp(smtp2)
                        tracker.record_send(domain)
                        health.record_success(from_email)
                        ...
                        _close_smtp(smtp2)
                errors_detail.append(
                    f"{mx_host}(aup_retry_exhausted): {code} {err_str[:80]}"
                )
                tracker.record_fail(domain, err_str)
                ...
                log.info(
                    "[MxSender] %s greylisted by %s — waiting %ds",
                    domain,
                    mx_host,
                    delay,
                )
                ...
                try:
                    ...
                    errors_detail.append(
                        f"{mx_host}(greylist-retry): {str(retry_exc)[:120]}"
                    )
