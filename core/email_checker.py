...
def _http_get_basic(
    ...
    return _http_get(
        url, {"Authorization": f"Basic {cred}", "Accept": "application/json"}
    )
...
class AWSSESProvider(EmailProvider):
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        try:
            ...
            status.extra_info["production_access"] = account.get(
                "ProductionAccessEnabled", False
            )
            status.extra_info["sending_enabled"] = account.get("SendingEnabled", False)
            status.extra_info["dedicated_ips"] = account.get(
        ...
        # Add region to extra info
        status.extra_info["region"] = self.region
        ...
        try:
            ...
            if domain_list:
                verif_resp = ses.get_identity_verification_attributes(
                    Identities=domain_list
                )
                dkim_resp = ses.get_identity_dkim_attributes(Identities=domain_list)

                # Fetch SPF (mail-from) and notification attributes
                try:
                    mailfrom_resp = ses.get_identity_mail_from_domain_attributes(
                        Identities=domain_list
                    )
                except Exception:
                    mailfrom_resp = {}
                try:
                    notif_resp = ses.get_identity_notification_attributes(
                        Identities=domain_list
                    )
                except Exception:
                    notif_resp = {}
                ...
                for domain in domain_list:
                    verif_attr = verif_resp.get("VerificationAttributes", {}).get(
                        domain, {}
                    )
                    dkim_attr = dkim_resp.get("DkimAttributes", {}).get(domain, {})
                    mailfrom_attr = (
                        mailfrom_resp.get("MailFromDomainAttributes") or {}
                    ).get(domain, {})
                    notif_attr = (notif_resp.get("NotificationAttributes") or {}).get(
                        domain, {}
                    )
                    domain_entry = {
                        "domain": domain,
                        ...
                        "dkim_status": dkim_attr.get(
                            "DkimVerificationStatus", "Unknown"
                        ),
                    }
                    if mailfrom_attr:
                        domain_entry["spf_status"] = mailfrom_attr.get(
                            "MailFromDomainStatus", "Unknown"
                        )
                        domain_entry["mail_from_domain"] = mailfrom_attr.get(
                            "MailFromDomain", ""
                        )
                    if notif_attr:
                        domain_entry["bounce_topic"] = bool(
                            notif_attr.get("BounceTopic")
                        )
                        domain_entry["complaint_topic"] = bool(
                            notif_attr.get("ComplaintTopic")
                        )
                    status.domains.append(domain_entry)
        except Exception as e:
            status.errors.append(f"Failed to get domains: {e}")

        # Get sending statistics (summed totals across all data points)
        try:
            stats_resp = ses.get_send_statistics()
            send_points = stats_resp.get("SendDataPoints", [])
            total_sent = total_bounces = total_complaints = total_rejects = 0
            for point in send_points:
                total_sent += point.get("DeliveryAttempts", 0)
                total_bounces += point.get("Bounces", 0)
                total_complaints += point.get("Complaints", 0)
                total_rejects += point.get("Rejects", 0)
            if send_points:
                status.extra_info["total_sent"] = total_sent
                status.extra_info["bounces"] = total_bounces
                status.extra_info["complaints"] = total_complaints
                status.extra_info["rejects"] = total_rejects
        except Exception:
            pass
...
class SendGridProvider(EmailProvider):
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        if data:
            ...
            status.extra_info["can_send_mail"] = (
                "✅ Yes" if can_send else "🚫 No — key missing mail.send scope"
            )
        ...
        if data:
            ...
            if status.monthly_limit and status.remaining_this_month is not None:
                status.sent_this_month = (
                    status.monthly_limit - status.remaining_this_month
                )
...
class MailgunProvider(EmailProvider):
    ...
    def detect(key: str, secret: Optional[str] = None) -> bool:
        ...
        return key.startswith("key-") or (
            len(key) >= 32 and re.match(r"^[a-f0-9-]+$", key)
        )
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        # Enrich each domain (up to 5) with DKIM/SPF status and SMTP credentials
        for domain_entry in status.domains[:5]:
            dname = domain_entry.get("domain")
            if not dname:
                continue
            try:
                ddata, _ = self._request(f"domains/{dname}")
                if ddata:
                    sending_records = (
                        ddata.get("domain", {}).get("sending_dns_records")
                        or ddata.get("sending_dns_records")
                        or []
                    )
                    dkim_status = "unknown"
                    spf_status = "unknown"
                    for rec in sending_records:
                        ...
                        value = (rec.get("value") or rec.get("data") or "").lower()
                        valid = rec.get("valid", "unknown")
                        if "domainkey" in name or "._domainkey" in name:
                            dkim_status = "valid" if valid == "valid" else valid
                        elif (
                            value.startswith("v=spf1")
                            or (rec.get("type") or "").upper() == "TXT"
                            and "spf" in value
                        ):
                            spf_status = "valid" if valid == "valid" else valid
                    domain_entry["dkim_status"] = dkim_status
                    domain_entry["spf_status"] = spf_status
            except Exception:
                pass
            # Fetch SMTP credentials and add logins to verified_emails
            try:
                cdata, _ = self._request(f"domains/{dname}/credentials")
                if cdata:
                    for cred in cdata.get("items") or []:
                        login = cred.get("login")
                        if login:
                            email_addr = (
                                f"{login}@{dname}" if "@" not in login else login
                            )
                            if email_addr not in status.verified_emails:
                                status.verified_emails.append(email_addr)
            except Exception:
                pass
        ...
        if self.domain:
            ...
            if data:
                ...
                if stats:
                    s = stats[0]
                    status.extra_info["delivered"] = s.get("delivered", {}).get(
                        "total", 0
                    )
                    status.extra_info["bounced"] = (
                        s.get("failed", {}).get("permanent", {}).get("total", 0)
                    )
...
class PostmarkProvider(EmailProvider):
    ...
    def _request(
        self, endpoint: str, account_token: bool = False
    ) -> Tuple[Optional[Dict], Optional[str]]:
        header_key = (
            "X-Postmark-Account-Token" if account_token else "X-Postmark-Server-Token"
        )
        return _http_get(
            f"{self.base_url}/{endpoint}",
    ...
    def fetch_status(self) -> EmailServiceStatus:
            ...
            for sender in data.get("SenderSignatures", []):
                if sender.get("Confirmed"):
                    ...
                    if domain and not any(
                        d["domain"] == domain for d in status.domains
                    ):
                        status.domains.append(
                            {
                                "domain": domain,
        ...
        # Get bounce stats breakdown
        data, _ = self._request("stats/outbound/bounces")
        if data:
            status.extra_info["total_bounces"] = data.get("TotalCount", 0)
            for bounce_type in data.get("Bounces") or []:
                if bounce_type.get("Type") == "HardBounce":
                    status.extra_info["hard_bounces"] = bounce_type.get("Count", 0)

        # Get suppression count
        data, _ = self._request("suppressions?count=1")
        if data:
            status.extra_info["suppression_count"] = data.get("TotalCount", 0)

        # Get unique opens
        data, _ = self._request("stats/outbound/opens")
        if data:
            status.extra_info["unique_opens"] = data.get("Unique", 0)
...
class MailjetProvider(EmailProvider):
    ...
    def _request(self, endpoint: str) -> Tuple[Optional[Dict], Optional[str]]:
        return _http_get_basic(
            f"{self.base_url}/{endpoint}", self.api_key, self.api_secret
        )
    ...
    def fetch_status(self) -> EmailServiceStatus:
            ...
            for key in data.get("Data", []):
                if key.get("APIKey") == self.api_key:
                    ...
                    status.account_status = (
                        "Active" if key.get("IsActive") else "Inactive"
                    )
        ...
        if data:
            for d in data.get("Data", []):
                status.domains.append(
                    {
                        "domain": d.get("Domain"),
                        "verified": not d.get("IsCheckInProgress")
                        and d.get("LastCheckAt") is not None,
        ...
        data, err = self._request(
            "statcounters?CounterSource=APIKey&CounterTiming=Message&CounterResolution=Lifetime"
        )
        if data:
            for stat in data.get("Data", []):
                ...
                status.extra_info["bounced"] = stat.get(
                    "MessageHardBouncedCount", 0
                ) + stat.get("MessageSoftBouncedCount", 0)

        # Get today's send stats
        try:
            today = datetime.utcnow().strftime("%Y-%m-%d")
            data, _ = self._request(
                f"statcounters?CounterSource=APIKey&CounterTiming=Message&CounterResolution=Day&FromTS={today}T00:00:00&ToTS={today}T23:59:59"
            )
            if data:
                for stat in data.get("Data", []):
                    status.sent_today = stat.get("MessageSentCount", 0)
                    status.extra_info["delivered_today"] = stat.get(
                        "MessageDeliveredCount", 0
                    )
        except Exception:
            pass

        # Get plan/subscription info from user endpoint
        try:
            udata, _ = self._request("user")
            if udata:
                user = udata.get("Data", [{}])[0]
                max_allowed = user.get("MaxAllowedMessages")
                if max_allowed:
                    status.monthly_limit = max_allowed
                status.extra_info["plan_code"] = user.get("SubscriptionType")
        except Exception:
            pass
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        if data:
            ...
            if plan_info:
                ...
                emails_per_month = (
                    plan.get("emailsPerMonth") or plan.get("maxCredits") or 0
                )
                ...
                if not status.monthly_limit:
                    status.monthly_limit = (
                        data.get("plan", {}).get("emailsPerMonth")
        ...
        today = datetime.utcnow().strftime("%Y-%m-%d")
        data, err = self._request(
            f"smtp/statistics/aggregatedReport?startDate={today}&endDate={today}"
        )
        if data:
            status.sent_today = data.get("requests", 0)
            status.extra_info["delivered_today"] = data.get("delivered", 0)
            status.extra_info["bounces_today"] = data.get("hardBounces", 0) + data.get(
                "softBounces", 0
            )
        ...
        if status.sent_this_month is None:
            month_start = datetime.utcnow().replace(day=1).strftime("%Y-%m-%d")
            data, err = self._request(
                f"smtp/statistics/aggregatedReport?startDate={month_start}&endDate={today}"
            )
            if data:
                status.sent_this_month = data.get("requests", 0)
                if status.monthly_limit and status.sent_this_month is not None:
                    status.remaining_this_month = max(
                        0, status.monthly_limit - status.sent_this_month
                    )
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        if data:
            ...
            if status.monthly_limit:
                status.remaining_this_month = status.monthly_limit - (
                    status.sent_this_month or 0
                )
            status.extra_info["service_level"] = results.get("service_level")
            status.extra_info["subscription"] = (results.get("subscription") or {}).get(
                "name"
            )
        elif err:
            status.errors.append(f"Account: {err}")

        # Get today's deliverability metrics
        try:
            today = datetime.utcnow().strftime("%Y-%m-%dT00:00")
            tomorrow = (datetime.utcnow() + timedelta(days=1)).strftime(
                "%Y-%m-%dT00:00"
            )
            data, _ = self._request(
                f"metrics/deliverability?precision=day&from={today}&to={tomorrow}&timezone=UTC"
            )
            if data:
                results_m = (data.get("results") or [{}])[0]
                status.sent_today = results_m.get("count_injected", 0)
                status.extra_info["delivered"] = results_m.get("count_delivered", 0)
                status.extra_info["bounces"] = results_m.get(
                    "count_hard_bounce", 0
                ) + results_m.get("count_soft_bounce", 0)
                status.extra_info["spam_complaints"] = results_m.get(
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        for d in (data or {}).get("data", []):
            ...
            dkim_ok = any(
                ...
                and "dkim" in (r.get("name") or "").lower()
                for r in records
            )
            spf_ok = any(
                (r.get("type") or "").upper() == "TXT" and r.get("status") == "verified"
                for r in records
            )
            status.domains.append(
                {
        ...
        # Fetch API key list to surface key metadata
        try:
            kdata, _ = self._request("api-keys")
            if kdata:
                keys = kdata.get("data", [])
                for k in keys:
                    name = k.get("name", "")
                    perm = k.get("permission", "")
                    if name:
                        status.extra_info["key_name"] = name
                        status.extra_info["key_permission"] = perm
                    break  # just first key
        except Exception:
            pass
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        if data:
            ...
            if pro.get("sends_allowance"):
                ...
                status.remaining_this_month = max(
                    0, status.monthly_limit - (status.sent_this_month or 0)
                )

        ldata, lerr = self._request(
            "lists?count=100&fields=lists.id,lists.name,lists.stats"
        )
        if ldata:
    ...
    def fetch_status(self) -> EmailServiceStatus:
        ...
        if err:
            status.account_status = (
                "Invalid Key"
                if (
                    "Invalid_Key" in (err or "")
                    or "Invalid API key" in (err or "")
                    or '"code": -1' in (err or "")
                )
                else "Error"
            )
        ...
        if data:
            ...
            status.extra_info["bounces_today"] = stats_today.get(
                "hard_bounces", 0
            ) + stats_today.get("soft_bounces", 0)
            ...
            status.extra_info["total_bounces"] = alltime.get(
                "hard_bounces", 0
            ) + alltime.get("soft_bounces", 0)
...
def format_report(status: EmailServiceStatus) -> str:
    ...
    lines.append("-" * 40)
    if (
        status.daily_limit is not None
        or status.sent_today is not None
        or status.sent_last_24h is not None
    ):
    ...
    if status.monthly_limit is not None or status.sent_this_month is not None:
        ...
        if status.remaining_this_month is not None:
            lines.append(
                f"  Remaining Month:  {format_number(status.remaining_this_month)}"
            )
...
def main():
    ...
    if not provider:
        ...
        if provider_class in (AWSSESProvider, MailjetProvider):
            print(
                f"Error: {provider_class.__name__.replace('Provider', '')} requires --secret"
            )
        else:
            print("Error: Could not detect provider from key format")
            print(
                "Supported: AWS SES, SendGrid, Mailgun, Postmark, Mailjet, Brevo, SparkPost"
            )
        sys.exit(1)
...
def _http_post(
    ...
    try:
        ...
        if (resp.headers.get("Content-Encoding") or "").lower() in ("gzip", "x-gzip"):
            try:
                raw_body = _gz.decompress(raw_body)
            except Exception:
                pass
        ...
        if (e.headers.get("Content-Encoding") or "").lower() in ("gzip", "x-gzip"):
            try:
                raw_err = _gz.decompress(raw_err)
            except Exception:
                pass
...
def add_domain(
    key: str, domain: str, secret: str = None, region: str = "us-east-1"
) -> Dict:
    """
    ...
    if key.startswith("key-") or (len(key) >= 32 and re.match(r"^[a-f0-9-]+$", key)):
        ...
        records = []
        for rec in (d2 or {}).get("sending_dns_records", []) + (d2 or {}).get(
    ...
    if key.startswith("xkeysib-") or key.startswith("xsmtpsib-"):
        ...
        if d2.get("dkim_record"):
            records.append(
                {
                    "type": "TXT",
                    "host": d2.get("dkim_selector", "brevo") + "._domainkey." + domain,
                    "value": d2["dkim_record"],
                    "label": "DKIM",
                }
            )
    ...
    return {
        "error": "Domain management not supported for this provider via API key format. "
...
def verify_domain(
    ...
    if key.startswith("SG."):
        ...
        if domain_id:
            ...
            if e2:
                return {"provider": "SendGrid", "domain": domain, "error": e2}
            match = next((d for d in (d2 or []) if d.get("domain") == domain), None)
            if not match:
                return {
                    "provider": "SendGrid",
                    "domain": domain,
                    "error": "Domain not found in account. Add it first.",
                }
            domain_id = match["id"]
            data, err = _http_post(
                f"https://api.sendgrid.com/v3/whitelabel/domains/{domain_id}/validate",
        ...
        return {
            ...
            if valid
            else "❌ Not verified — check DNS records are propagated (can take up to 48h)",
        }
    ...
    if key.startswith("key-") or (len(key) >= 32 and re.match(r"^[a-f0-9-]+$", key)):
        ...
        all_valid = (
            all(r.get("valid") == "valid" for r in sending) if sending else False
        )
        return {
            "provider": "Mailgun",
            "domain": domain,
            ...
            "records": [
                {
                    "label": r.get("record_type", "") + " " + r.get("name", ""),
                    "valid": r.get("valid") == "valid",
                    "reason": r.get("value", ""),
                }
                for r in sending
            ],
            "message": "✅ Domain verified!"
            if all_valid
            else "❌ Some DNS records not yet valid",
        }
    ...
    if key.startswith("xkeysib-") or key.startswith("xsmtpsib-"):
        ...
        return {
            ...
            "message": "✅ Domain verified!"
            if verified
            else "❌ Not verified — check DNS records",
        }
