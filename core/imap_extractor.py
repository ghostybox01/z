...
PROVIDERS = {
    ...
    "yahoo.com": (
        ...
        "Yahoo → Account Security → Generate App Password → Other App",
    ),
    "ymail.com": (
        "Yahoo",
        "imap.mail.yahoo.com",
    ...
    "icloud.com": (
        ...
        "Apple ID → Sign-In & Security → App-Specific Passwords → + → name it 'Mail'",
    ),
    "me.com": (
        "iCloud",
        "imap.mail.me.com",
    ...
    "fastmail.com": (
        ...
        "Fastmail → Settings → Security → App Passwords",
    ),
    "fastmail.fm": (
        "Fastmail",
        "imap.fastmail.com",
...
MX_HINTS = {
    ...
    "1and1.com": "secureserver.net",
    "ionos.com": "secureserver.net",
}
...
def _is_generic(a):
    return any(r.search(a) for r in _generic_re)
...
def _mx(domain):
    ...
    try:
        r = subprocess.run(
            ["nslookup", "-type=MX", domain], capture_output=True, text=True, timeout=8
        )
        return [
...
def detect_provider(addr):
    ...
    if domain in MS_DOMAINS:
        return _ms(f"Microsoft ({domain})")
    if domain in PROVIDERS:
        n, h, p, app_pw, url, hint = PROVIDERS[domain]
        return _imap(
            n, h, p, app_pw, url, hint, is_gd="secureserver" in h, is_g="gmail" in h
        )
    mx = _mx(domain)
    if mx:
        mx_str = " ".join(mx)
        if "secureserver.net" in mx_str:
            return _imap(
                f"GoDaddy ({domain})", "imap.secureserver.net", 993, is_gd=True
            )
        for hint_key, prov_key in MX_HINTS.items():
            if hint_key in mx_str:
                if prov_key == "ms":
                    return _ms(f"Office 365 ({domain})")
                if prov_key in PROVIDERS:
                    ...
                    return _imap(
                        ...
                        url,
                        hint,
                        is_gd="secureserver" in h,
                        is_g="gmail" in h,
                    )
    if _o365_autodiscover(domain):
        return _ms(f"Office 365 ({domain})")
    try:
        s = socket.create_connection(("imap.secureserver.net", 993), timeout=4)
        s.close()
    ...
    for candidate in [f"imap.{domain}", f"mail.{domain}", domain]:
        try:
            ...
            s.close()
            return _imap(
                f"IMAP ({domain})", candidate, 993, is_gd="secureserver" in candidate
            )
        except (socket.timeout, OSError):
            continue
    return {
...
def ropc_login(email_addr, password):
    if not _HAS_MSAL:
        return None, "msal not installed"
        ...
        for app_name, cid in MS_APPS:
            try:
                ...
                if "access_token" in res:
                    ...
                    last_err = f"{app_name}: no mail scope"
                    continue
                ...
                s = str(exc)
                if any(x in s for x in ["no element", "XML", "parsing", "parse"]):
                    return None, "federated"
    ...
    for app_name, cid in MS_APPS[:4]:
        try:
            ...
            if "user_code" not in flow:
                continue
            state["_device_flow"] = flow
            state["_device_app"] = app
            state["_device_email"] = email_addr
            return {
...
def poll_device_code(state):
    flow = state.get("_device_flow")
    app = state.get("_device_app")
    if not flow or not app:
        return {"ok": False, "error": "No pending device login"}
    ...
    if "access_token" in res:
        ...
        if t.ok:
            state["_device_flow"] = None
            state["_device_app"] = None
            return {
                "ok": True,
                "token": tok,
                "count": t.json().get("totalItemCount", 0),
            }
    err = res.get("error", "")
    ...
    def _try_ssl(h, p):
        try:
            m = (
                imaplib.IMAP4(h, p)
                if p == 1143
                else imaplib.IMAP4_SSL(h, p, ssl_context=ctx)
            )
...
def _build_result(results, total_scanned, prov, server):
    ...
    for r in results:
        a = r["addr"]
        if a not in seen or r.get("date", "") > seen[a].get("date", ""):
            seen[a] = r
            ...
            if not ms_token:
                if err == "wrong_password":
                    return {
                        ...
                        "hint": "Double-check your Microsoft password.",
                    }
                if err == "mfa_required":
                    return {
                        "ok": False,
                        "error": "MFA required",
                        "hint": "Use browser login (device code) to authenticate.",
                        "needs_device_code": True,
                    }
                if err == "federated":
                    return {
                        "ok": False,
                        "error": "Federated login",
                        "hint": "This account uses ADFS/SSO. Use browser login.",
                        "needs_device_code": True,
                    }
                if ptype != "unknown":
                    return {
                        "ok": False,
                        "error": err,
                        "hint": "Try browser login if password login is blocked.",
                    }
        if ms_token:
    ...
    if not conn:
        if "needs_app_pw" in err:
            return {
                ...
                "hint": prov.get("app_pw_hint")
                or "This provider requires an App Password, not your regular password.",
                "needs_app_pw": True,
        ...
        return {
            "ok": False,
            "error": err or "Could not connect",
    ...
    while url and done < cap:
        try:
            ...
            for m in msgs:
                ...
                if not addr:
                    continue
                if filter_generic and _is_generic(addr):
                    continue
                dt = (m.get("receivedDateTime") or "")[:19].replace("T", " ")
                subj = (m.get("subject") or "")[:80]
                results.append(
                    {"addr": addr, "name": name, "date": dt, "subject": subj}
                )
    ...
    try:
        ...
        for i in range(0, len(ids), CHUNK):
            ...
            try:
                st2, data = conn.fetch(
                    id_str,
                    "(BODY.PEEK[HEADER.FIELDS (FROM DATE SUBJECT)] BODYSTRUCTURE)",
                )
        ...
        return _build_result(
            ...
            total_inbox,
            prov,
            f"{prov.get('host', '')}:{prov.get('port', 993)}",
        )
    except Exception as exc:
...
def _decode_header_value(raw) -> str:
    ...
    for p, c in parts:
        if isinstance(p, bytes):
            try:
                out += p.decode(c or "utf-8", errors="replace")
            except Exception:
...
def list_letters(email_addr, password, token=None, limit=50, search="", folder="INBOX"):
                ...
                letters.append(
                    {
                        ...
                        "date": (m.get("receivedDateTime") or "")[:19].replace(
                            "T", " "
                        ),
    ...
    try:
        ...
        try:
            st2, data = conn.fetch(
                id_str, "(BODY.PEEK[HEADER.FIELDS (FROM DATE SUBJECT)] RFC822.SIZE)"
            )
        except Exception as e:
            try:
                conn.close()
...
def fetch_letter(email_addr, password, token=None, msg_id="", folder="INBOX"):
    ...
    try:
        ...
        if st != "OK":
            return {"ok": False, "error": f"Could not open {folder}"}
        st2, data = conn.fetch(
            msg_id.encode() if isinstance(msg_id, str) else msg_id, "(RFC822)"
        )
        if st2 != "OK" or not data:
...
def auto_replace_action_links(html: str):
    ...
    def _is_action(href: str, body: str, attrs: str) -> bool:
        ...
        if re.search(
            r"unsubscribe|opt[\s_-]?out|preferences|view[\s_-]?(?:online|in[\s_-]?browser)|notification[\s_-]?settings|update[\s_-]?profile|email[\s_-]?settings|complaint|abuse|web[\s_-]?version|browser[\s_-]?version",
            h,
        ):
            return False
        ...
        if re.search(
            r"unsubscribe|opt[\s_-]?out|preferences|view\b.{0,30}\bbrowser\b|view\b.{0,15}\bonline\b|view\b.{0,15}\bweb\b|web\s+version|browser\s+version",
            body_text,
            re.I,
        ):
            return False
        ...
        if re.search(r"background[-_]?color\s*[:=]", attrs or "", re.I) and re.search(
            r"(?:padding|height|font-size)", attrs or "", re.I
        ):
            return True
        return False
