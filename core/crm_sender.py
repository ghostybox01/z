...
def _post_json(
    url: str, payload: dict, headers: dict, provider: str, timeout: int = 30
) -> int:
    ...
    for attempt in range(_MAX_RETRIES + 1):
        req = Request(
            url, data=raw, headers={**headers, "Content-Type": "application/json"}
        )
        try:
            ...
            if exc.code in (429, 503, 502) and attempt < _MAX_RETRIES:
                ...
                try:
                    ...
                    delay = _RETRY_DELAY
                log.warning(
                    "[CrmSender] %s HTTP %d — retrying in %.0fs",
                    provider,
                    exc.code,
                    delay,
                )
...
def _send_hubspot(crm_cfg, sender, lead, html, plain, subject, i):
    ...
    if template:
        ...
        url = (
            endpoint
            or "https://api.hubapi.com/marketing/v3/transactional/single-email/send"
        )
        payload = {
        ...
        payload = {
            "properties": {
                ...
                "hs_email_to_lastname": to_name.split()[-1]
...
def _sf_get_token(crm_cfg: dict) -> tuple:
    ...
    if not username:
        ...
        return crm_cfg.get("apiKey", ""), crm_cfg.get(
...
def _send_salesforce(crm_cfg, sender, lead, html, plain, subject, i):
    ...
    if not instance_url:
        raise Exception(
            "Salesforce: instanceUrl not configured and OAuth did not return one."
        )
...
def _send_dynamics(crm_cfg, sender, lead, html, plain, subject, i):
    ...
    try:
        ...
        if exc.code == 401:
            raise Exception(
                "Dynamics 401 — invalid access token. Obtain a fresh token from Azure AD."
            )
    ...
    send_url = (
        f"{org_url}/api/data/v9.2/emails({email_id})/Microsoft.Dynamics.CRM.SendEmail"
    )
...
def _send_zoho(crm_cfg, sender, lead, html, plain, subject, i):
    ...
    return _post_json(
        url, payload, {"Authorization": f"Zoho-oauthtoken {token}"}, "zoho"
    )
...
def _send_pipedrive(crm_cfg, sender, lead, html, plain, subject, i):
    ...
    url = (
        endpoint
        or f"https://api.pipedrive.com/v1/mailbox/mailMessages?api_token={api_key}"
    )
