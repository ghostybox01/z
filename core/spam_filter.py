...
def _get_patterns():
    ...
    _PATTERNS = [
        (
            re.compile(
                r"(?<![a-zA-Z])" + re.escape(k) + r"(?![a-zA-Z])", re.IGNORECASE
            ),
            v,
        )
        for k, v in items
...
def _walk_text_nodes(html, fn):
    ...
    while i < len(html):
        ...
        if end == -1:
            out.append(fn(html[i:]))
            break
        out.append(fn(html[i:end]))
        i = end
    return "".join(out)
...
def inject_zero_font(html, intensity=2):
    ...
    if "<head>" in html.lower():
        ...
        html = re.sub(
            r"(<body[^>]*>)", _ZF_CSS + r"\1", html, flags=re.IGNORECASE, count=1
        )
    else:
        html = _ZF_CSS + html
...
def apply_homoglyph_encoding(text, rate=0.4):
    ...
    return re.sub(
        r"\b[a-zA-Z]+\b",
        lambda m: (
            _hg_word(m.group(0), rate)
            if m.group(0).lower() in _HG_TARGETS
            else m.group(0)
        ),
        text,
    )
...
def apply_homoglyph_html(html, rate=0.4):
    return (
        _walk_text_nodes(html, lambda t: apply_homoglyph_encoding(t, rate))
        if html
        else html
    )
...
_FULL_WIDTH_MAP = {
    c: chr(ord(c) + 0xFEE0) for c in string.printable[:-5] if " " < c < "~"
}
...
def encode_subject(text: str, method: int = ENC_NONE) -> str:
    ...
    try:
        if method == ENC_NONE:
            return text
        if method == ENC_QP:
            return _encode_qp(text)
        if method == ENC_BASE64:
            return _encode_b64_word(text)
        if method == ENC_HOMOGLYPH:
            return apply_homoglyph_encoding(text)
        if method == ENC_ZERO_WIDTH:
            return inject_invisible_chars(text)
        if method == ENC_UNICODE_NORM:
            return _encode_unicode_norm(text)
        if method == ENC_MIXED_SCRIPT:
            return _encode_mixed_script(text)
        if method == ENC_MATH_ALPHA:
            return _encode_math_alpha(text, bold_sans=False)
        if method == ENC_FULL_WIDTH:
            return _encode_full_width(text)
        if method == ENC_ADVANCED_ZWJ:
            return _encode_advanced_zwj(text)
        if method == ENC_COMBINING:
            return _encode_combining(text)
        if method == ENC_MATH_BOLD:
            return _encode_math_alpha(text, bold_sans=True)
    except Exception:
        pass
...
def inject_noise_pixel(html):
    ...
    px = (
        f'<img src="data:image/gif;base64,{_GIF1x1}" '
