...
def generate_template(api_key: str, theme: str, category: str = "business",
...
def _parse_response(text: str) -> dict:
    ...
    if html_match:
        ...
        raw_match = re.search(r'(<!DOCTYPE html.*?</html>)', text, re.DOTALL | re.IGNORECASE)
        if raw_match:
    ...
    if subj_match:
        lines = [l.strip().lstrip("0123456789.-) ") for l in subj_match.group(1).splitlines()]
        subjects = [l for l in lines if l][:5]
